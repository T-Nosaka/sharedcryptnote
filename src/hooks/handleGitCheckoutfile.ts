...
export type CheckoutFileMode = 'ours' | 'theirs' | 'hash';
...
export const useHandleGitCheckoutHashFile = (
  setMessage: (message: string) => void,
  setLoading: (loading: boolean) => void,
  callback?: () => void
) => {
  return async (gitinfostr: string, mode: CheckoutFileMode, selectFilePath: string, hash?: string) => {
    ...
    try {
        ...
        const response = await fetch('/api/git/checkoutfile', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ gitinfostr: gitinfostr, mode: mode, selectFilePath:selectFilePath, hash:hash })
        });
        const data = await response.json();
        if (response.ok) {
            if (callback)
